...
import { getStore } from '@/framework/client/helper';
...
const Popup: React.FC<ProductPopupProps> = ({ productSlug }) => {
  ...
  const { product, isLoading } = useProduct({ slug: productSlug  , store: getStore() });
  ...
  if (isLoading || !product)
    return (
      <div className="relative flex items-center justify-center w-96 h-96 bg-light">
        <Spinner text={STATIC_CONTENT['text-loading']} />
      </div>
    );
