...
const Neon: React.FC<NeonProps> = ({ product, className }) => {
  ...
  const { name, image, quantity, min_price, max_price, product_type , available } =
    product ?? {};
  const { price, basePrice, discount } = usePrice({
    amount: product.sale_price ? product.sale_price : product.price!,
    baseAmount: product.price,
          ...
          <>
            {(Number(quantity) > 0 && available) && (
              ...
              >
                ...
                <span className="w-7 h-7 md:w-9 md:h-9 bg-gray-200 grid place-items-center ltr:rounded-tr rtl:rounded-tl ltr:rounded-br rtl:rounded-bl transition-colors duration-200 group-hover:bg-accent-hover group-hover:text-light-hover group-focus:bg-accent-600">
