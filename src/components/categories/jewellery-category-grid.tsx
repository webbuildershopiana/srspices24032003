...
import JewelleryCategoryCard from './jewellery-category-card';
...
interface JewelleryCategoryGridProps {
  notFound: boolean;
  loading: boolean;
  categories: Category[];
  className?: string;
...
const JewelleryCategoryGrid: React.FC<JewelleryCategoryGridProps> = ({
  notFound,
  categories,
  loading,
  variables,
  ...
  return ( 
    <div className="relative">
      <div className="mx-auto w-full ">
        <div className="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-5 lg:gap-8 xl:gap-14">
          {Array.isArray(renderCategories) &&
            categories.slice(0, 5)?.map((item: any, idx: number) => (
              <JewelleryCategoryCard key={item.id} item={item} />
...
export default JewelleryCategoryGrid;
{
